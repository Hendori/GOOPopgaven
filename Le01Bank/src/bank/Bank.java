...
public class Bank {
  ...
  public int maakSpaarrekening(String naam, int tegen) {
    ...
    if (rtegen != null && rtegen instanceof Betaalrekening) {
      int nummer = nieuwRekeningNummer();
      Rekeninghouder rekeninghouder = new Rekeninghouder(naam);
      Spaarrekening rekening = new Spaarrekening(rekeninghouder, nummer, (Betaalrekening)rtegen);
      rekeningen.add(rekening);
      return nummer;
    }
   ...
   * @param van rekeningnummer van de rekening waarvan geld wordt overgemaakt (moet betaalrekening zijn)
   ...
   * moeten worden uitgevoerd. Dat hoeft alleen voor spaarrekeningen.
   */
  public void eindeMaand() {
    for (Rekening r : rekeningen) {
      if (r instanceof Spaarrekening) {
        ((Spaarrekening)r).eindeMaand();
      }
    }
  }
